...
import com.floreantpos.report.CustomPaymentReportModel;
...
public class CustomPaymentReportView
extends JPanel {
    ...
    public CustomPaymentReportView() {
        ...
        this.btnGo.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    CustomPaymentReportView.this.viewReport();
                }
                catch (Exception e1) {
    ...
    private void viewReport() throws Exception {
        ...
        List<? extends PosTransaction> transactions = PosTransactionDAO.getInstance().findTransactions(CustomPaymentTransaction.class, fromDate, toDate);
        int saleCount = 0;
        double totalSales = 0.0;
        double totalTips = 0.0;
        for (CustomPaymentTransaction customPaymentTransaction : transactions) {
            ++saleCount;
            totalSales += customPaymentTransaction.getAmount().doubleValue();
            totalTips += customPaymentTransaction.getTipsAmount().doubleValue();
        }
        HashMap<String, String> map = new HashMap<String, String>();
        ReportUtil.populateRestaurantProperties(map);
        map.put("reportTitle", POSConstants.CUSTOM_PAYMENT_REPORT.toUpperCase());
        ...
        JasperReport jasperReport = ReportUtil.getReport("custom-payment-report");
        JasperPrint jasperPrint = JasperFillManager.fillReport((JasperReport)jasperReport, map, (JRDataSource)new JRTableModelDataSource((TableModel)new CustomPaymentReportModel(transactions)));
