...
std::string default_receipt_prompt_template()
{
    return
        ...
        "- Historical suppliers/products are not receipt data. Never copy a historical supplier, item, quantity, unit price, or total into the output unless the same supplier/item row is visibly supported by the current receipt image.\n"
        "- If a visible row is too degraded to align its values, leave uncertain numeric fields as `0` and add a warning instead of filling them from history.\n"
        ...
        "- For tables with headers like `CODE`, `DESIGNATION`, `PU_TTC`, `NBRE`, `P.UNITE`, `TOTAL HT`, `%`, and `VAT`: use `DESIGNATION` as the item name, `NBRE` as `quantity`, `P.UNITE` as `unit_price`, `TOTAL HT` as `total_price`, and row `VAT` as `line_tax_amount`. `PU_TTC` is a tax-inclusive/display unit price, not quantity. `%` is a tax rate, not a unit price.\n"
        "- For those `TOTAL HT` tables, use the footer `TOTAL VT HT`/subtotal as `purchase_order.subtotal_amount`, the footer VAT total as `purchase_order.tax_amount`, and `NET PAYABLE TTC` as `purchase_order.grand_total`; set `line_total_basis` to `exclusive`.\n"
        ...
        "- For H.X.M/HXM invoice tables with columns like `Pack`, `CTN`, `Quantity`, `Init Price`, and `Amount (VT)`, use `Quantity` as `quantity`, `Init Price` as `unit_price`, and `Amount (VT)` as `total_price`. `Pack` and `CTN` are not item quantity unless no `Quantity` column exists.\n"
        "- Do not use the footer `TOTAL` row's CTN/quantity values as any item's quantity, unit price, or amount.\n"
        ...
        "- If a quantity continuation line such as `qty 2 @ 375/t` belongs to the item printed immediately above it, keep it with that item and use quantity `2`, unit_price `375`, and total_price from that same item group.\n"
        ...
        "- Payment rows such as cash, card, eftpos, paid, tendered, received, balance, and change are payment details, not purchase items and not header totals.\n"
        ...
        "- Vanuatu uses Vatu (VT/VUV), which has no cents in normal receipt totals.\n"
        "- Receipts may mix decimal quantities such as `0.64` with whole-currency money amounts such as `822`, `8,240`, `1,286`, `12,044`, `2.450`, or `21.230`.\n"
        ...
        "- If the printed money amount is `2.450`, output `2450`.\n"
        "- If the printed money amount is `4.900`, output `4900`.\n"
        "- If the printed money amount is `21.230`, output `21230`.\n"
        "- If the printed amount is `11,64`, output `11.64`.\n"
        "- If the printed amount is `11.64`, output `11.64`.\n"
        "- For money values, a dot followed by exactly three digits is a thousands separator, not a decimal point. A decimal dot can only have one or two following digits.\n"
        ...
        "- For POS receipts, the payable amount is the printed total/subtotal due before payment is tendered. Do not use cash/card tendered, amount paid, balance, or change as `grand_total`, `subtotal_amount`, `discount_amount`, or any item value.\n"
        "- If a footer shows `TOTAL 9,195vt`, then `CASH 10,000vt`, then `CHANGE 805vt`, set `grand_total` to `9195` and ignore the cash and change values except for an optional warning.\n"
        ...
        "- A footer value labeled `Total Tax`, `VAT`, or `Tax` is receipt-level `purchase_order.tax_amount`; do not copy that footer tax total into item `line_tax_amount` unless the receipt also shows a per-line tax column.\n"
        "- If line item totals sum to a printed pre-tax subtotal and the footer shows a separate total tax, set `line_total_basis` to `exclusive` and put the footer tax in `purchase_order.tax_amount`.\n"
        "- For VAT invoices whose item lines show pre-tax `VT` line amounts and whose footer shows `Sub-total`, `Discount`, `Total Tax`, and `Total`, use the item-line sum as `subtotal_amount`, the printed `Total Tax` as `tax_amount`, and the printed `Total` as `grand_total`; set `discount_amount` to `0` when the discount line is absent or printed as `VT0.00`.\n"
        "- Do not treat the trailing digits of a subtotal as a discount. For example, `Sub-total VT7,466.90` means subtotal `7466.90`, not discount `466.90`.\n"
        "- Ignore cash/payment rounding shown in the tender/payment section when subtotal plus receipt-level tax already equals `grand_total`; do not put that payment-only adjustment in `rounding_amount`.\n"
        "- Phrases such as `you saved`, `customer savings`, `loyalty savings`, or `special savings` are not `discount_amount` unless a printed discount line directly participates in the receipt arithmetic. If item totals already add up to the printed subtotal/total, keep `discount_amount` as `0`.\n"
        "- If line totals already include tax, set `line_total_basis` to `inclusive`.\n"
        "- If the receipt says the invoice includes VAT/tax but does not show a separate tax amount, set `tax_amount` to `0` and `line_total_basis` to `inclusive`.\n"
        ...
        "- Before returning JSON, compare the sum of purchase item `total_price` values against the printed subtotal/total. If it does not match, re-read the lowest-confidence rows; never change `grand_total` to a cash tender amount just to make totals balance.\n\n"
        "Category rules:\n"
